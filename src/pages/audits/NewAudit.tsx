...
const MOCK_MATERIALS = [
  { id: "m1", code: "RM-001", name: "Basmati Rice", category: "Grains", primaryUnit: "KG", currentStock: 120 },
  { id: "m2", code: "RM-002", name: "Olive Oil (Extra Virgin)", category: "Oils", primaryUnit: "LTR", currentStock: 45 },
  { id: "m3", code: "RM-003", name: "Chicken Breast", category: "Meat", primaryUnit: "KG", currentStock: 30 },
  { id: "m4", code: "RM-004", name: "Onion (Red)", category: "Vegetables", primaryUnit: "KG", currentStock: 200 },
  { id: "m5", code: "RM-005", name: "Tomato Paste", category: "Sauces", primaryUnit: "KG", currentStock: 60 },
  { id: "m6", code: "RM-006", name: "Cumin Powder", category: "Spices", primaryUnit: "KG", currentStock: 15 },
  { id: "m7", code: "RM-007", name: "Mozzarella Cheese", category: "Dairy", primaryUnit: "KG", currentStock: 25 },
  { id: "m8", code: "RM-008", name: "All-Purpose Flour", category: "Grains", primaryUnit: "KG", currentStock: 300 },
];
...
export default function NewAudit() {
  ...
  const [confirmAction, setConfirmAction] = useState<null | "draft" | "template" | "update">(null);
  ...
  const addMaterials = (ids: string[]) => {
    ...
    const newItems = ids.filter((id) => !existing.has(id)).map((id) => {
      ...
      return { id: m.id, code: m.code, name: m.name, category: m.category, stockCount: m.currentStock, totalCount: 0, batch: "" };
    });
    setItems((prev) => [...prev, ...newItems]);
  };

  const removeItem = (id: string) => setItems((prev) => prev.filter((i) => i.id !== id));
  const updateItem = (id: string, updates: Partial<AuditLineItem>) => setItems((prev) => prev.map((i) => i.id === id ? { ...i, ...updates } : i));

  const handleConfirm = () => {
    if (confirmAction === "draft") {
      toast({ title: "Audit Saved", description: "Audit saved as draft." });
      navigate("/audits", { state: { selectTab: "Draft" } });
    } else if (confirmAction === "template") {
      toast({ title: "Template Saved", description: "Audit saved as template." });
      navigate("/audits", { state: { selectTab: "templates" } });
    } else if (confirmAction === "update") {
      toast({ title: "Inventory Updated", description: "Audit has been submitted for review." });
      navigate("/audits", { state: { selectTab: "InReview" } });
    }
  ...
  return (
    <div className="space-y-4 max-w-[1200px]">
      <div>
        <h1 className="cento-page-title text-xl">New Audit</h1>
        <p className="text-sm text-muted-foreground mt-0.5">Create a new inventory audit</p>
      </div>
            ...
            <TableBody>
              {filteredItems.length === 0 ? (
                ...
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-right">{item.stockCount}</TableCell>
                  <TableCell className="text-right">
                    <Input type="number" className="h-8 w-20 text-xs text-right ml-auto" value={item.totalCount || ""} onChange={(e) => updateItem(item.id, { totalCount: Number(e.target.value) || 0 })} />
                  </TableCell>
                  <TableCell>
      ...
      <ConfirmationModal
        ...
        description="Clicking on Confirm will save this audit as a draft."
        onConfirm={handleConfirm}
        confirmLabel="Confirm"
      />
      <ConfirmationModal
        open={confirmAction === "template"}
        onOpenChange={() => setConfirmAction(null)}
        title="Save Template Confirmation"
        description="Clicking on Confirm will save this audit as a template."
        onConfirm={handleConfirm}
        confirmLabel="Confirm"
      />
      <ConfirmationModal
        ...
        onOpenChange={() => setConfirmAction(null)}
        title="Update Inventory Confirmation"
        description="Clicking on Confirm will submit this audit for review and update the inventory."
        onConfirm={handleConfirm}
        confirmLabel="Confirm"
      />
    </div>
